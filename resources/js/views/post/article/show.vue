                        ...
                        <slot name="body">
                            <h6 class="card-title">
                                ...
                                <p class="pull-right" v-if="article.user">
                                    <strong>{{trans('post.article_posted_by')}}:</strong> {{getEmployeeName(article.user.employee)}}
                                </p>
                            </h6>
                            <div class="m-t-20 html-view" v-html="article.description"></div>
...
<!--                                        <a :href="`/post/article/${article.uuid}/attachment/${attachment.uuid}/download?token=${authToken}`" class="no-link-color"><i :class="['file-icon', 'fas', 'fa-lg', attachment.file_info.icon]"></i> <span class="upload-file-list-item-size">{{attachment.file_info.size}}</span> {{attachment.user_filename}}</a>-->
                                        <a :href="`/storage/${attachment.filename}`" class="no-link-color" target="_blank"><i :class="['file-icon', 'fas', 'fa-lg', attachment.file_info.icon]"></i> <span class="upload-file-list-item-size">{{attachment.file_info.size}}</span> {{attachment.user_filename}}</a>
                                    </li>
                                </ul>
                            </div>
                            <hr />
